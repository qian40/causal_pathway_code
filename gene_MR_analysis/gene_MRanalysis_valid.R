# eQTL result
eqtl = read.table("eQTL/Whole_Blood.v8.EUR.signif_pairs.txt",sep="\t",quote="",header=TRUE,stringsAsFactors=FALSE)
...
library(ieugwasr)
library(plinkbinr)
clump_data = data.frame(rsid=eqtl$rsid,id=eqtl$gene_name,pval=eqtl$pval_nominal)
result = ld_clump(dat=clump_data[1:1600004,],clump_kb=2000,clump_r2=0.01,clump_p=1,plink_bin=get_plink_exe(),bfile="LDpruning/EUR")
clump_result = result
...
library(stringr)
mrdata$tmp = mrdata$variant_id
mrdata[,c("chr","bp","A1","A2","type")] = str_split_fixed(mrdata$tmp,"_",n=5)
...
over3_adhd_mrdata = subset(adhd_mrdata,adhd_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
...
over_3 = subset(count_result,count_result$n>=1)
over3_bipolar_mrdata = subset(bipolar_mrdata,bipolar_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
exposure_dat = format_data(
...
over_3 = subset(count_result,count_result$n>=1)
over3_schizo_mrdata = subset(schizo_mrdata,schizo_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
exposure_dat = format_data(
  over3_schizo_mrdata,
...
over_3 = subset(count_result,count_result$n>=1)
over3_depression_mrdata = subset(depression_mrdata,depression_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
exposure_dat = format_data(
...
over_3 = subset(count_result,count_result$n>=1)
over3_insomnia_mrdata = subset(insomnia_mrdata,insomnia_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
exposure_dat = format_data(
...
over_3 = subset(count_result,count_result$n>=1)
over3_autism_mrdata = subset(autism_mrdata,autism_mrdata$gene_name %in% over_3$gene_name)
# MR analysis
library(TwoSampleMR)
exposure_dat = format_data(
  over3_autism_mrdata,
